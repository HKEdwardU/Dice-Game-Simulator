...
def dicegame_Sim(NUM_ROLLS=5000):
    ...
    res_count_df = pd.DataFrame(res_count, columns=["DiceFace-TotalCount", "count"])
    res_Diceface_df = pd.DataFrame(res_Diceface, columns=["DiceFace-ShowAtLeastOnce", "count"])
    res_sum_count_df = pd.DataFrame(res_sum_count, columns=["Sum", "count"])
    print('\n', res_count_df, '\n', res_Diceface_df, '\n', res_sum_count_df, '\n', all_same)

    res_count_df = res_count_df.set_index('DiceFace-TotalCount')
    res_count_df.plot(kind='bar')
    plt.xlabel("Dice Face")
    plt.ylabel("Count")
    plt.show()

    res_Diceface_df = res_Diceface_df.set_index('DiceFace-ShowAtLeastOnce')
...
Times = int(input('Times for dice roll sim(keep empty for default amount):'))
if Times:
    dicegame_Sim(Times)
else:
    dicegame_Sim()
